...
from ..model import *
from .model import *
from .dataset import IEMOCAPDataset, SynthesisDataset, collate_fn
import pdb
...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, downstream_expert, expdir, **kwargs):
        ...
        DATA_ROOT_syn = self.datarc['syn_root']
        DATA_ROOT_iemo = self.datarc['iem_root'] #'/mnt/Internal'
        ...
        train_path = os.path.join(
            meta_data, self.fold, 'train_meta_data.json')
        print(f'[Expert] - Training path: {train_path}')

        test_path = os.path.join(
            meta_data, self.fold, 'test_meta_data.json')
        print(f'[Expert] - Testing path: {test_path}')
        
        dataset = SynthesisDataset(DATA_ROOT_syn, train_path, self.datarc['pre_load'])
        if self.datarc['valid_ratio']:
            ...
            dev_path = os.path.join(meta_data, self.fold, 'dev_meta_data.json')
            print(f'[Expert] - Dev path: {dev_path}')
            self.train_dataset = dataset
            self.dev_dataset = SynthesisDataset(DATA_ROOT_syn, dev_path, self.datarc['pre_load'])
        self.test_dataset = IEMOCAPDataset(DATA_ROOT_iemo, test_path, self.datarc['pre_load'])
    ...
    def get_downstream_name(self):
        return self.fold.replace('prompt1', 'zero_shot_emotion')
    ...
    def log_records(self, mode, records, logger, global_step, **kwargs):
        ...
        if mode in ["dev", "test"]:
            with open(Path(self.expdir) / f"{mode}_{self.fold.replace('/', '_')}_predict.txt", "w") as file:
            # with open(Path(self.expdir) / f"{mode}_{self.fold}_predict.txt", "w") as file:
                line = [f"{f} {e}\n" for f, e in zip(records["filename"], records["predict"])]
                file.writelines(line)

            with open(Path(self.expdir) / f"{mode}_{self.fold.replace('/', '_')}_truth.txt", "w") as file:
